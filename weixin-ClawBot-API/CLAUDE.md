...
### AI API (`callAI`)
`callAI()` function sends `POST <base_url>/v1/messages` with `Authorization: Bearer` header. Built-in gradient retry (5 retries: 2s/4s/8s/16s/32s). Parses Claude (`content[0].text`) vs GPT (search `content` for `type: "text"`) response formats.
